...
import Amount from './amount'
  ...
  export default class Fk extends Component {
    constructor(props){
        super(props);
        this.state={
          lists_hotel_info: [],lists_hotel_city:[],lists_hotel_score:[],
            lists_film_info:[],lists_film_company:[],lists_film_genre_level:[],lists_film_info2:[],lists_film_company2:[],lists_film_genre_level2:[],
            lists_world_city:[],lists_world_country:[],lists_world_countrylanguage:[],

            hotel_info:'',hotel_city:'',hotel_score:'',
            film_info:'',film_company:'',film_genre_level:'',film_info2:'',film_company2:'',film_genre_level2:'',
    ...
    toShopDetails = (id,table_name) => {
        ...
        var film_info_list = [];
        var film_info2_list = [];
        var film_company2_list = [];
        var film_genre_level2_list = [];
      ...
      if(table_name ==='world_countrylanguage'){
        var rref = ref.child('world/world_countrylanguage')
        rref.orderByChild("CountryCode").equalTo(String(id)).on("value", function(snapshot) {
          snapshot.forEach(function(childSnapshot) {
            var childData_2 = snapshot.val();
            var start = Number(Object.keys(childData_2)[0]);var end = Number(Object.keys(childData_2)[0]) + Object.keys(childData_2).length-1
      ...
      if(table_name ==='film_company2'){
        ...
        rref.orderByChild("company").equalTo(String(id)).on("value", function(snapshot) {
          snapshot.forEach(function(childSnapshot) {
            ...
            for (var j = start; j <= end;j++){
                if(typeof(childData_2[j])!='undefined'){
                    film_company2_list[j] = {company_id:childData_2[j].company_id,company:childData_2[j].company,country:childData_2[j].country}
      ...
      if(table_name ==='film_genre_level2'){
        var rref = ref.child('film/film_genre_level')
        ...
        rref.orderByChild("id").equalTo(String(id)).on("value", function(snapshot) {
          snapshot.forEach(function(childSnapshot) {
            ...
            for (var j = start; j <= end;j++){
                if(typeof(childData_2[j])!='undefined'){
                    film_genre_level2_list[j] = {id:childData_2[j].id,genre:childData_2[j].genre,rating:childData_2[j].rating}
      ...
      if(world_country_list.length!== 0){
        ...
        if(world_city_list.length !== 0){
             ...
             if(world_countrylanguage_list.length !== 0){
                ...
                if(film_genre_level2_list.length !== 0){
                    this.setState({ lists_film_genre2_level:film_genre_level2_list,
                      film_genre_level2:'film_genre_level2' }
                    ); 
                  }
                  if(film_company2_list.length !== 0){
                    this.setState({ lists_film_company2:film_company2_list,
                      film_company2:'film_company2' }
                    ); 
                }
    }
      render() {
        ...
        if (this.state.world_city === 'world_city'){    
            return(
                    <Pk lists={this.state.lists_world_city} key_1={this.state.world_city} fb={this.props.fb}/>
        ...
        if (this.state.world_country === 'world_country'){ 
            console.log('')   
            return(
                <Pk lists={this.state.lists_world_country} key_1={this.state.world_country} fb={this.props.fb}/>
                                        
                          )       
                          }
          if (this.state.world_countrylanguage === 'world_countrylanguage'){   
                            return(
                                <Pk lists={this.state.lists_world_countrylanguage} key_1={this.state.world_countrylanguage} fb={this.props.fb}/>
         ...
         if (this.state.hotel_city === 'hotel_city'){    
                  return(
              <Pk lists={this.state.lists_hotel_city} key_1={this.state.hotel_city} fb={this.props.fb}/>
                                                                
            )       
             }
             if (this.state.hotel_info === 'hotel_info'){    
                return(
            <Pk lists={this.state.lists_hotel_info} key_1={this.state.hotel_info} fb={this.props.fb}/>
                                                              
          )       
           }
           if (this.state.film_info === 'film_info'){    
            return(
        <Pk lists={this.state.lists_film_info} key_1={this.state.film_info} fb={this.props.fb}/>
                                                          
      )       
       }
       if (this.state.film_info2 === 'film_info2'){    
        return(
    <Pk lists={this.state.lists_film_info2} key_1='film_info' fb={this.props.fb}/>
                                                      
  )       
   }
   if (this.state.film_company === 'film_company'){    
    return(
<Pk lists={this.state.lists_film_company} key_1='film_company' fb={this.props.fb}/>
                                                  
)       
}
if (this.state.film_genre_level === 'film_genre_level'){    
    return(
<Pk lists={this.state.lists_film_genre_level} key_1='film_genre_level' fb={this.props.fb}/>
                                                  
)       
}
if (this.state.film_company2 === 'film_company2'){    
    return(
<Pk lists={this.state.lists_film_company2} key_1='film_company' fb={this.props.fb}/>
                                                  
)       
}
if (this.state.film_genre_level2 === 'film_genre_level2'){    
    return(
<Pk lists={this.state.lists_film_genre_level2} key_1='film_genre_level' fb={this.props.fb}/>
     ...
     if(this.props.key_1 === 'world_country2'){
        return (
          <div>
          <div className="container" style={{marginTop:'30px'}}>
          <h4 className="text-center ">Table: world_country</h4>  
                <h6 className="text-center ">Primary key:&nbsp;Code</h6>
                <h6 className="text-center ">The amount of data:&nbsp;<Amount lists ={this.props.lists} /></h6>
                <br/><table className="table table-striped table-hover">
                  <thead>
              ...
              <tbody>
                ...
                return (         
                  <tr key={i} className="text-center">
                  <td><a style={{color:'CornflowerBlue'}} onClick={() => this.toShopDetails(list.Code,'world_countrylanguage')}>{list.Code}</a></td>
                  <td>{list.Name}</td>
                  <td>{list.Continent}</td>
                  <td>{list.Region}</td>
                  <td>{list.SurfaceArea}</td>
                  ...
                  <tr className='warning'>
                    ...
                    <tbody>
                      ...
                      return (         
                        ...
                        <td>{list.film_id}</td>
                        <td>{list.name}</td>
                        <td><a style={{color:'CornflowerBlue'}}onClick={() => this.toShopDetails(list.company,"film_company2")}>{list.company}</a></td>
                        <td>{list.director}</td>
                        <td><a style={{color:'CornflowerBlue'}}onClick={() => this.toShopDetails(list.genre_level,"film_genre_level2")}>{list.genre_level}</a></td>
                        <td>{list.runtime}</td>
                        <td>{list.score}</td>
                        <td>{list.votes}</td>
                        <td>{list.star}</td>
